...
export default class RoleIndex extends base {
  ...
  worldCardData(worldExplorations, maxData = {}) {
    ...
    for (let id of lodash.orderBy(cardIds, [v => Number(v)], ["desc"])) {
      ...
      cards.push({
        ...
        gradient: theme.from ? `linear-gradient(103deg, ${theme.from} 50%, ${theme.to} 100%)` : "",
        levels,
        percents,
        areas,
  ...
  roleExploreSrData(res) {
    ...
    const region =
      role.region_name ||
      regionMap[role.region] ||
      role.region ||
      uidRegionMap[uidPrefix] ||
      "星穹列车"
    ...
    const data = {
      ...
      game_head_icon:
        ...
        role.avatar_url ||
        role.avatar?.icon ||
        resIndex?.cur_head_icon_url ||
        `${this.screenData.pluResPath}img/role/派遣头像.png`,
    ...
    const line = [
      ...
      [
        { lable: "宝箱总数", num: total.cur, extra: total.max },
        {
          lable: "已完成世界",
          num: worlds.filter(
            world => toNum(world.world_max) > 0 && toNum(world.world_cur) >= toNum(world.world_max),
          ).length,
        },
        {
          lable: "地图区域",
          num: worlds.reduce((sum, world) => sum + (world.map_entrances?.length || 0), 0),
        },
  ...
  async bgImage() {
    let res = await fetch(
      "https://fastcdn.mihoyo.com/mi18n/hk4e_cn/m20240919hy3a2yr11c/m20240919hy3a2yr11c-zh-cn.json",
    )
    res = await res.json()
  ...
  async roleExploreData(res) {
    ...
    let line = [
      ...
      [
        ...
        { lable: "冰神瞳", num: stats.iceculus_number, extra: this.lable.cryoculus },
      ],
    ]
    // 尘歌壶
    ...
    let worldCards =
      exploreStyle === 1 ? this.worldCardData(resIndex.world_explorations, maxData) : []
