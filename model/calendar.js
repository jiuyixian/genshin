...
export default class Calendar {
  ...
  async getGs(e) {
    let r = await MysInfo.get(e, "act_calendar")
    if (!r || r.retcode) return false
    return { game: "gs", ...this.fGs(r.data) }
  }
  async getSr(e) {
    let r = await MysInfo.get(e, "act_calendar")
    if (!r || r.retcode) return false
    return { game: "sr", ...this.fSr(r.data) }
  }
  async getZzz(e) {
  ...
  fGs(d) {
    const n = Date.now()
    return W(
      d.avatar_card_pool_list?.[0]?.version_name || "",
      (d.avatar_card_pool_list || []).map(b =>
        B(
          b.pool_name || "",
          b.version_name || "",
          dt(b.start_timestamp),
          dt(b.end_timestamp),
          n,
          (b.avatars || []).map(a => ({
            name: a.name,
          ...
          (b.weapon || []).map(w => ({
            name: w.name,
            icon: w.icon,
            rarity: typeof w.rarity === "number" ? w.rarity : 5,
          })),
          b.pool_status,
        ),
      ),
      (d.weapon_card_pool_list || []).map(b =>
        B(
          b.pool_name || "",
          b.version_name || "",
          dt(b.start_timestamp),
      ...
      [...(d.act_list || []), ...(d.fixed_act_list || [])].map(e => {
        const s = dt(e.start_timestamp, 1),
          ed = dt(e.end_timestamp, 1)
        return E(
          e.name,
          s,
          ed,
          n,
          (e.reward_list || [])
            .filter(r => r.homepage_show)
            .map(r => ({ name: r.name, icon: r.icon, num: r.num })),
          e.tower_detail || null,
          e.role_combat_detail || null,
          e.hard_challenge_detail || null,
          undefined,
          undefined,
          undefined,
          undefined,
  ...
  fSr(d) {
    const n = Date.now()
    return W(
      d.cur_game_version || d.avatar_card_pool_list?.[0]?.version || "",
      (d.avatar_card_pool_list || []).map(b => {
        const t = b.time_info || {}
        return B(
          b.name || "",
      ...
      [
        ...(d.act_list || []).map(e => {
          const t = e.time_info || {}
          const s = dt(t.start_ts),
            ed = dt(t.end_ts)
          const sm = {
            DoubleRewardActStatusProgress: 1,
            DoubleRewardActStatusUnopened: 0,
            OtherActStatusUnopened: 0,
            OtherActStatusFinish: 2,
            SignStatusFinish: 2,
          }
          const Rs = []
          const sp = e.special_reward
          if (sp?.item_id && sp.name && sp.num)
            Rs.push({ name: sp.name, icon: sp.icon, num: sp.num })
          return E(
            e.name,
            s,
            ed,
        ...
        ...(d.challenge_list || []).map(c => {
          ...
          const tn =
            {
              ChallengeTypePeak: "异相仲裁",
              ChallengeTypeChasm: "混沌回忆",
              ChallengeTypeStory: "虚构叙事",
              ChallengeTypeBoss: "末日幻影",
            }[c.challenge_type] || ""
          const Rs = []
          const sp = c.special_reward
          if (sp?.item_id && sp.name && sp.num)
            Rs.push({ name: sp.name, icon: sp.icon, num: sp.num })
          return E(
            `${tn ? tn + " · " : ""}${c.name_mi18n || ""}`,
  ...
  fZzz(gd, ad) {
    const n = Date.now(),
      R = { S: 5, A: 4 }
    return W(
      gd.avatar_gacha_schedule_list?.[0]?.version || "",
      (gd.avatar_gacha_schedule_list || []).map(b =>
        B(
          "角色活动调频",
          b.version || "",
          dt(b.start_ts),
          ...
          (b.avatar_list || []).map(a => ({
            ...
            icon: a.icon,
            rarity: R[a.rarity] || 4,
            element: zE(a.avatar_element_type),
          })),
          [],
        ),
      ),
      (gd.weapon_gacha_schedule_list || []).map(b =>
        B(
          "音擎活动调频",
          b.version || "",
          dt(b.start_ts),
          ...
          [],
          (b.weapon_list || []).map(w => ({
            name: w.talent_title || w.weapon_name || w.name || "音擎",
            icon: w.icon,
            rarity: R[w.rarity] || 4,
          })),
        ),
      ...
      (ad.activity_list || []).map(e => {
        ...
        if (e.monochrome_cnt)
          Rs.push({
            name: "菲林",
            icon: "https://act.mihoyo.com/app/mihoyo-zzz-game-record/images/icon-feilin-large.7850ba22.png",
            num: e.monochrome_cnt,
          })
...
function B(name, version, st, et, n, A, W, ps) {
  const f5 = A.filter(a => a.rarity === 5),
    f4 = A.filter(a => a.rarity === 4)
  const w5 = W.filter(w => w.rarity === 5),
    w4 = W.filter(w => w.rarity === 4)
  ...
  return {
    ...
    hasChars: f5.length + f4.length > 0,
    hasWeapons: w5.length + w4.length > 0,
  }
}
...
function E(name, st, et, n, Rs, tower, rc, hc, showText, cp, tp, actSc, gsStatus) {
  ...
  return {
    ...
    towerDetail: tower || null,
    roleCombatDetail: rc || null,
    hardChallengeDetail: hcDetail || null,
    showText: showText || "",
    curProg: cp != null ? cp : -1,
    totalProg: tp != null ? tp : -1,
  }
}
...
function dt(v, sZ) {
  if (!v) return null
  const n = +v
  return !n || (sZ && n === 0) ? null : new Date(n * 1000)
}
function fmt(d) {
  if (!d) return ""
...
function zE(t) {
  const m = { 200: "physical", 201: "fire", 202: "ice", 203: "electro", 204: "wind", 205: "ether" }
  return m[t] || ""
}
